<?php

require_once("header.php");

?>
<nav class="navbar">
  <div class="searchs">
    <a href="#"><img class='logo' src="../img/facebook.jpg" alt=""></a>
    <input class="search" type="text" name='search' placeholder="Type for search...">
      <i class='fas fa-search fass' style='font-size:24px'></i>
  </div>
  <div class="icon">
      <i class='fas fa-user-friends fass' style='font-size:24px;'></i>
      <i class='fas fa-home  fass' style='font-size:24px;'></i>
      <i class='far fa-user-circle fass fas' style='font-size:24px;'></i>
  <a href=""><img class='logo' src="../img/facebook.jpg" alt=""></a>
  <div class="searchs">
    <input class="search" type="text" name='search' placeholder="Type for search...">
      <i class='fas fa-search' style='font-size:24px'></i>
  </div>
  <div class="icon">
    <i class='fas fa-home' style='font-size:24px;cursor:pointer;'></i>
    <i class='far fa-user-circle fas' style='font-size:24px;cursor:pointer;'></i>
      <i class='fas fa-user-friends' style='font-size:24px;cursor:pointer;'></i>
  </div>
</nav> 

